...
// relative to +ext.x
hinge_inset_dx = hinge_chamfer_dx - pivot_hinge.x;
...
module shell_long(ext, int) {
  ...
  module mask_stitches() {
    ...
    dx = ext.x / 2 - hole_stitch_inset - hinge_inset_stitches;
...
module shell_main() {
  difference() {
    union() {
      ...
      shell_long(ext=ext_main, int=int_main);
    }
